...
private:
	...
	uint8_t u8lastRec;
	uint16_t *_inputRegs, *_holdingRegs;
	...
	void get_FC1();
	void get_FC3();
	int8_t process_FC1(uint16_t regs); // Read Coils
	int8_t process_FC3(uint16_t *regs, uint8_t u8size); // Read Holding Registers
	int8_t process_FC5(uint16_t &regs); // Write Single Coil
	int8_t process_FC6(uint16_t *regs, uint8_t u8size); // Write Single Register
	int8_t process_FC15(uint16_t &regs); //Write Multiple Coils
	int8_t process_FC16(uint16_t *regs, uint8_t u8size); //Write Multiple registers
	// Р—Р°РїРёСЃСЊ РІ EEPROM
	int8_t process_FC21(); //Write File Record
...
public:
  ...
  //   int8_t poll(); //!<cyclic poll for master
	int8_t poll(uint16_t &discreteInputs, uint16_t &coils, uint16_t *inputRegs, const uint8_t inputRegsCount, uint16_t *holdingRegs, const uint8_t holdingRegsCount); //!<cyclic poll for slave
 ...
 * @ingroup loop
 */
int8_t Modbus::poll(uint16_t &discreteInputs, uint16_t &coils, uint16_t *inputRegs, const uint8_t inputRegsCount, uint16_t *holdingRegs, const uint8_t holdingRegsCount) {
	...
	auto u8exception = validateRequest();
	if (u8exception > 0) {
	...
	case MB_FC_READ_COILS:
		return process_FC1(coils);
		break;
	case MB_FC_READ_DISCRETE_INPUT:
		return process_FC1(discreteInputs);
		break;
	case MB_FC_READ_REGISTERS:
		return process_FC3(holdingRegs, holdingRegsCount);
		break;
	case MB_FC_READ_INPUT_REGISTER:
		return process_FC3(inputRegs, inputRegsCount);
		break;
	case MB_FC_WRITE_COIL:
		return process_FC5(coils);
		break;
	case MB_FC_WRITE_REGISTER:
		return process_FC6(holdingRegs, holdingRegsCount);
		break;
	case MB_FC_WRITE_MULTIPLE_COILS:
		return process_FC15(coils);
		break;
	case MB_FC_WRITE_MULTIPLE_REGISTERS:
		return process_FC16(holdingRegs, holdingRegsCount);
		break;
...
uint8_t Modbus::validateRequest() {
	...
	case MB_FC_WRITE_MULTIPLE_COILS:
		...
		u16count = word(au8Buffer[NB_HI], au8Buffer[NB_LO]);
		if (u16count > 16)
	...
	case MB_FC_WRITE_MULTIPLE_REGISTERS:
		...
		// Запись в EEPROM  
	case MB_FC_WRITE_FILE_RECORD:
		if (au8Buffer[FILE_REF_TYPE] != 6)
			return EXC_ADDR_RANGE;
		// Номер файла должен быть 0x0001
		if (au8Buffer[FILE_NUM_HI] != 0x00 || au8Buffer[FILE_NUM_LO] != 0x01)
			return EXC_ADDR_RANGE;
		// Проверка чтобы не выходило за пределы EEPROM
		unsigned long startAddrBytes = ((au8Buffer[FILE_REC_HI] << 8) | au8Buffer[FILE_REC_LO]) << 1;
		unsigned long recLenBytes = ((au8Buffer[FILE_REC_LEN_HI] << 8) | au8Buffer[FILE_REC_LEN_LO]) << 1;
		if (startAddrBytes + recLenBytes >= E2END)
			return EXC_ADDR_RANGE;
		break;
	}
 ...
 * @ingroup register
 */
int8_t Modbus::process_FC21() {
	uint8_t u8func = au8Buffer[FUNC];  // get the original FUNC code

	uint8_t requestDataLen = au8Buffer[FILE_DATA_LEN];

	uint16_t startAddrsBytes = (word(au8Buffer[FILE_REC_HI], au8Buffer[FILE_REC_LO])) << 1;
	uint16_t recLenBytes = (word(au8Buffer[FILE_REC_LEN_HI], au8Buffer[FILE_REC_LEN_LO])) << 1;
	...
	//au8Buffer[ NB_HI ]   = 0;
	//au8Buffer[ NB_LO ]   = u8regsno;
	u8BufferSize = requestDataLen + 1;


	// write EEPROM
	for (i = 0; i < recLenBytes; i++) {
		EEPROM[startAddrsBytes + i] = au8Buffer[FILE_FIRST_BYTE + i];
	}
